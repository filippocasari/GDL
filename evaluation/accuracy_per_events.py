...
from torchmetrics.functional.classification.accuracy import accuracy as acc
def parse_args():
...
def evaluate(model, data_loader: Iterable[Batch], max_num_events: int) -> float:
    ...
    for i, batch in enumerate(data_loader):
        ...
        accuracy_i = acc(preds=y_hat_i, target=sample.y).cpu().numpy()#pl_metrics.accuracy(preds=y_hat_i, target=sample.y).cpu().numpy()
        accuracy.append(accuracy_i)
    print(f"accuracy mean {float(np.mean(accuracy))}")
    return float(np.mean(accuracy))


def main(args, model, data_module):
    max_num_events = np.arange(1000, 5000, step=500)
    df = pd.DataFrame()

    output_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "..", "aegnn_results")
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f"accuracy_per_events_{args.dataset}.pkl")

    for max_count in tqdm(max_num_events):
        data_loader = data_module.val_dataloader(num_workers=8).__iter__()
